		...
		$('.open-vform-callback').click(function(){
			$(".bl-forms").queue(function () { // добавим в очередь
				...
				$.ajax({
					...
					url: '/bitrix/templates/otoplenie/ajax/callback.php',
					cache: false,
					dataType: 'html'
				}).done(function(html) {
		...
		$(document).on('submit','form#vform-callback' , function(){
			...
			if($('#cf_phone').val() == ""){
				...
				if(!selection){
					$.ajax({
					method: 'post',
					data: ({'action':'vbtn-send','name':$('#vf_fio').val(),'phone':$('#vf_tel').val()}),
					url: '/bitrix/templates/otoplenie/ajax/callback.php',
					cache: false,
					dataType: 'html'
				}).done(function(html) {
		...
		$('.open-vform-raschet').click(function(){
			$(".bl-forms").queue(function () { // добавим в очередь
				...
				$.ajax({
					...
					url: '/bitrix/templates/otoplenie/ajax/raschet.php',
					cache: false,
					dataType: 'html'
				}).done(function(html) {
					$('.bl-forms-wrap').html(html);
		...
		$('.open-vform-zakaz').click(function(){
			$(".bl-forms").queue(function () { // добавим в очередь
				...
				$.ajax({
					...
					url: '/bitrix/templates/otoplenie/ajax/zakaz.php',
					cache: false,
					dataType: 'html'
				}).done(function(html) {
					$('.bl-forms-wrap').html(html);
		...
		$(document).on('submit','form#form-vopros' , function(){
			...
			if($('#cf_phone').val() == ""){
				...
				if(!selection){
					...
					url: '/bitrix/templates/otoplenie/ajax/vopros.php',
					cache: false,
					dataType: 'html'
				}).done(function(html) {
					$('.faq-form')
		...
		$(document).on('submit','form#form-otziv' , function(){
			...
			if($('#cf_phone').val() == ""){
				...
				if(!selection){
					...
					method: 'post',
					data: ({'action':'btn-send','name':$('#cf_fio').val(),'phone':$('#cf_mail').val(),'comment':$('#cf_comm').val()}),
					url: '/bitrix/templates/otoplenie/ajax/otzivi.php',
					cache: false,
					dataType: 'html'
				}).done(function(html) {
					$('.otziv-form')
