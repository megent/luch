...
?>
<p>
Запрошенная страница не найдена, возможно изменился её адрес.<br>
Воспользуйтесь картой сайта для поиска нужной страницы:
</p>
<table class="map-columns">
<tr>
	<td>
		<ul class="map-level-0">
			<li><a href="%lnk%/">Главная страница</a></li>
		...
		foreach($arResult["arMap"] as $index => $arItem):?>
			...
			<?if (array_key_exists($index+1, $arResult["arMap"]) && $arItem["LEVEL"] < $arResult["arMap"][$index+1]["LEVEL"]):?>
				...
				<li><a href="%lnk%<?=$arItem["FULL_PATH"]?>"><?=$arItem["NAME"]?></a><?if ($arParams["SHOW_DESCRIPTION"] == "Y" && strlen($arItem["DESCRIPTION"]) > 0) {?><div><?=$arItem["DESCRIPTION"]?></div><?}?>
					<ul class="map-level-<?=$arItem["LEVEL"]+1?>">

			<?else:?>

					<li><a href="%lnk%<?=$arItem["FULL_PATH"]?>"><?=$arItem["NAME"]?></a><?if ($arParams["SHOW_DESCRIPTION"] == "Y" && strlen($arItem["DESCRIPTION"]) > 0) {?><div><?=$arItem["DESCRIPTION"]?></div><?}?></li>
