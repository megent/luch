...
$(document).ready(function() {
	$('span.extlink') //обработка внешних ссылок. внешние ссылки оформлять ввиде "<span class="extlink" data-href="куда_ведет_ссылка">текст_ссылки</span>
		.click( function(){
			window.open($(this).attr("data-href"), '_blank');
		});
		
	$('span.inlink') //обработка внутренних ссылок. внешние ссылки оформлять ввиде "<span class="inlink" data-href="куда_ведет_ссылка">текст_ссылки</span>
		.click( function(){
			window.open($(this).attr("data-href"), '_self');
		});
		
	//Открытие списка городов
	$("#active-city").click(function(event) {
		$('#city-list').fadeIn(200);
	});
	$('.close').click(function(){
			$("#city-list").fadeOut(200);
		}
	);
	...
	$('.climaline-slick').slick({
		prevArrow: '<button type="button" data-role="none" class="clim-prev" aria-label="previous"></button>',
		nextArrow: '<button type="button" data-role="none" class="clim-next" aria-label="next"></button>'
	});
	$('.carusel-cert').slick({
		infinite: false,
		slidesToShow: 3,
